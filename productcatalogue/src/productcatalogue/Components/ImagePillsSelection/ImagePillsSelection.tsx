  ...
  const handleDeletingImage=(image:ImageOptions)=>{
    ...
    return (
        <div className='Item-Selection'>
            ...
            /> <span><img src={Searchicon} alt=""  className='searchicon'/></span>
                    ...
                    <ul className='Selected-Images'>
                        {selectedImages.map(image => (
                            <li key={image.id} className='Selected-Image-Item'>
                               <img  src={`/assets/${image.imageId}.${image.imageType && image.imageType.split('/')[1]}`}   alt="img" />

                                <span>{image.name}</span>
                                <img src={deleteIcon} alt="" onClick={()=>handleDeletingImage(image)} className='Selected-Image-Item-Deletion'/>
                            </li>
                        ))}
                    </ul>
                </div>
